  ...
  useEffect(() => {
    ...
    if (randomRotation && cards.length > 0) {
      setRandomRotations(prev => {
        ...
        // Only generate rotations for cards that don't already have them
        cards.forEach(card => {
          if (!(card.id in rotations)) {
  ...
  return (
    ...
    >
      {cards.map((card, index) => {
        const randomRotate = randomRotation && isMounted ? (randomRotations[card.id] || 0) : 0;
