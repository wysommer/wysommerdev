...
const RippleGrid: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    // Wait for next frame to ensure container has valid dimensions
    const initTimeout = setTimeout(() => {
      ...
      initializeRenderer(clientWidth, clientHeight);
    }, 10);
...
void main() {
    ...
    return () => {
      ...
      if (mouseInteraction && handleMouseMove && handleMouseEnter && handleMouseLeave && containerRef.current) {
  ...
  return <div ref={containerRef} className="w-full h-screen relative overflow-hidden [&_canvas]:block" />;
};
